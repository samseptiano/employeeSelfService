...
public class KPIApproveListAdapter extends RecyclerView.Adapter<KPIApproveListAdapter.MyViewHolder> {
    ...
    public KPIApproveListAdapter(List<KPIApproveListPJ> KPIList, Context context, Activity activity, EventListener listener,String empType) {
    ...
    public void onBindViewHolder(KPIApproveListAdapter.MyViewHolder holder, int position) {
        try {
            ...
            userList.setMulaiKontrak(klist.getMulaiKontrak());
            userList.setSelesaiKontrak(klist.getSelesaiKontrak());
                        ...
                        rootRef.addListenerForSingleValueEvent(new ValueEventListener() {
                            ...
                            public void onDataChange(@NonNull com.google.firebase.database.DataSnapshot dataSnapshot) {
                                ...
                                if (dataSnapshot.hasChild(nik+"-"+ friendNIK+"-"+nama+"-"+friendName+"-YEAR")) {
                                    ...
                                    routingHomeDetailInterface.routingChat(nik+"-"+ friendNIK+"-"+nama+"-"+friendName+"-YEAR",friendName, friendNIK,semester,tahun,kualitatif,kuantitatif,context, KPIItems,friendBranchName,friendDept,friendJobTitle,foto,"LIST",empType);

                                }
                                else if(dataSnapshot.hasChild(friendNIK+"-"+nik+"-"+friendName+"-"+nama+"-YEAR")){
                                    RoutingHomeDetailInterface routingHomeDetailInterface = new RoutingHomeDetail();
                                    routingHomeDetailInterface.routingChat(friendNIK +"-"+nik+"-"+friendName+"-"+nama+"-YEAR",friendName,friendNIK,semester,tahun,kualitatif,kuantitatif,context,KPIItems,friendBranchName,friendDept,friendJobTitle,foto,"LIST",empType);
                                }
                                else{
...
//        holder.tvDept.setText(klist.getDept()+" - "+klist.getBranchName()+" - "+klist.getJobStatus());
            ...
            holder.tvEmpPeriode.setText(klist.getMulaiKontrak().split(" ")[0]+ " - " + klist.getSelesaiKontrak().split(" ")[0]);
            holder.lnApprove.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Fragment fr = new KPIKuantitatifFragment();
                    ...
                    bundle6.putString("tahun",tahun);
                    bundle6.putString("semester","1");
                    bundle6.putString("empType",empType);
                    ...
                    Fragment frr = new TabLayoutFragment();
