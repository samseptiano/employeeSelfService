...
import com.enseval.samuelseptiano.hcservice.Model.APIModel.SetupPA.PASetting.PASettingDetail;
...
public class PositionSetupDetailPAAdapter extends RecyclerView.Adapter<PositionSetupDetailPAAdapter.MyViewHolder> {
    ...
    private List<PASettingHeader> paSettingHeaderList = new ArrayList<>();
    ...
    public interface EventListener {
        ...
        List <PASettingHeader> getPASettingHeader();
        void setPaSettingHeaderList(List<PASettingHeader> paSettingHeaderList);
...
//            Toast.makeText(context,Integer.toString(position)+" "+questions.get(position-1).getKPIcategory()+" "+category,Toast.LENGTH_SHORT).show();
    ...
    public PositionSetupDetailPAAdapter(List<SetupPositionDetailModel> setupPositionDetailModelList, Context context, Activity activity, EventListener listener, List<PASettingHeader> paSettingHeaderList,String type ) {
    ...
    @Override
    public PositionSetupDetailPAAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.setup_pa_item_list, parent, false);

        return new PositionSetupDetailPAAdapter.MyViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(PositionSetupDetailPAAdapter.MyViewHolder holder, int position) {
        ...
        holder.cbSetup.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if(b){
                    ...
                    listener.setPaSettingHeaderList(paSettingHeaderList);
...
//    }
    ...
    private void showAddDialog(String title, SetupPositionDetailModel setupEmployeeDetailModel, int pos,String numb){
        ...
        String tempSemester = setupEmployeeDetailModel.getSemester();
...
//                }
        ...
        btnSaveKPI.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                    ...
                    //validasi pindah semester


                    if(!spinnerSemester.getText().toString().contains(tempSemester)){
                            for(int i=0;i<setupPositionDetailModelList.size();i++){
                                if(setupPositionDetailModelList.get(i).getSemester().equals(setupPositionDetailModel.getSemester()) &&
                                        setupPositionDetailModelList.get(i).getKPIDesc().equals(setupPositionDetailModel.getKPIDesc())){
                    ...
                    if(ctr==0){
                        ...
                        //for (int i = 0; i < setupPositionDetailModelList.size(); i++) {
                            ...
                            paSettingDetail.setSemester(semester[0]);
                            paSettingDetail.setKpiID(setupEmployeeDetailModel.getId());
                        ...
                        Call<Void> call = apiService.postPAKPISettingHeaderOne(paSettingHeaders, "Bearer " + usr.get(0).getToken());
                        call.enqueue(new Callback<Void>() {
                            @Override
                            public void onResponse(Call<Void> call, Response<Void> response) {
                                int statusCode = response.code();
                                Toast.makeText(context, "Template Disimpan", Toast.LENGTH_LONG).show();
