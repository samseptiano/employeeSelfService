...
import com.enseval.samuelseptiano.hcservice.Adapter.RecyclerViewAdapter.KPIAdapterTahunan.ListDevPlanAdapter;
import com.enseval.samuelseptiano.hcservice.Adapter.RecyclerViewAdapter.KPIAdapterTahunan.ListImprovementAreaFullScreenAdapter;
...
import com.enseval.samuelseptiano.hcservice.Model.APIModel.KPI.PA.IDPLayoutListModel;
...
public class DevelopmentPlanDetailFragment extends Fragment implements ListImprovementAreaFullScreenAdapter.EventListener, ListDevPlanAdapter.EventListener {
    ...
    String empName, jobTitle, dept, branchName, jobStatus, empPhoto, empNik,canApprove,apprseq,compid="",fromGotoIDP="",compName="", orgname="";
                        ...
                        try {
                            ...
                            btnApproveDevplan.setOnClickListener(new View.OnClickListener() {
                                ...
                                public void onClick(View v) {
                                    ...
                                    try {
                                        if (kpiHeader.getDevPlanHeaderList().size() > 0) {
                                            for (int i = 0; i < kpiHeader.getDevPlanHeaderList().size(); i++) {
                                                if(kpiHeader.getDevPlanHeaderList().get(0).getDevPlanDetail().size()>0) {
                                                    for (int j = 0; j < kpiHeader.getDevPlanHeaderList().get(i).getDevPlanDetail().size(); j++) {
                                                        if (kpiHeader.getDevPlanHeaderList().get(i).getDevPlanDetail().get(j).getDEVPLANACTIVITIES().equals("") ||
                                                                ...
                                                                kpiHeader.getDevPlanHeaderList().get(i).getDevPlanDetail().get(j).getDEVPLANMENTOR().equals("") ||
                                                                kpiHeader.getDevPlanHeaderList().get(i).getDevPlanDetail().get(j).getDEVPLANACHIEVEMENT().equals("")
                                                        ) {
                                                            ctr++;
                                                        }
                                                    }
                                            ...
                                            public void onResponse(Call<KPIAnswerList> call2, Response<KPIAnswerList> response) {
                                                ...
                                                call.enqueue(new Callback<Void>() {
                                                    ...
                                                    public void onResponse(Call<Void> call, Response<Void> response) {
                                                        ...
                                                        //sendWhatsapp("62"+kpiHeader.getNomorAtasan2().substring(1,kpiHeader.getNomorAtasan2().length()-1),"test wa");

                                                        EmailSentModel emailSentModel = new EmailSentModel();
                                                        emailSentModel.SUBJECT="[HCService] IDP karyawan atas nama "+empName+" menunggu approval";
                                                        emailSentModel.MESSAGE="<br /> <br /> Bersama ini kami informasikan, IDP untuk karyawan dibawah ini menunggu approval Bapak/Ibu</p>\n" +
                                                                ...
                                                                "</tbody>\n" +
                                                                "</table>\n" +
                                                                "<p><br /> Approval IDP dapat dilakukan melalui aplikasi HC Service yang dapat diakses <strong>melalui HC Service web <a href=\"http://hc.enseval.com:8443\">(klik disini)</a> atau HC Service Mobile. </strong> <br /> <br /> Terima kasih <br /> <br /> <em>(Note: email ini adalah hasil generate otomatis dari aplikasi HC Service)</em></p>";
