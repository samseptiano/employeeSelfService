package com.enseval.samuelseptiano.hcservice.Fragment.TalentManagementFragment;
...
public class TalentManagementFragment extends Fragment implements IconHomeAdapter.EventListener{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        rootView = inflater.inflate(R.layout.fragment_talent_management, container, false);

        ((AppCompatActivity)getActivity()).getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        ((AppCompatActivity)getActivity()).getSupportActionBar().setDisplayShowCustomEnabled(false);
        ((AppCompatActivity)getActivity()).getSupportActionBar().setTitle("Talent Management");
        ...
        rvHcPerformance = rootView.findViewById(R.id.rv_icon_talent_management);
        rvHcPerformance.setLayoutManager(new GridLayoutManager(getContext(), 3));
        for(int i=0;i<mdl.size();i++) {
            if(mdl.get(i).getModuleCode().equals("HCRM") || mdl.get(i).getModuleCode().equals("HCAS")
                    || mdl.get(i).getModuleCode().equals("ENGMNPRFL")) {
    ...
    public void callAction(String moduleCode) {
        switch(moduleCode) {
            case "HCRM":
                ...
                ft.addToBackStack("HCRoadmapFragment");
                ft.commit();
                Toast.makeText(getContext(),"HC Roadmap Area",Toast.LENGTH_SHORT).show();
                break;
            case "HCAS":
                break;
            case "ENGMNPRFL":
